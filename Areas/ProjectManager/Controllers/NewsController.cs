...
namespace project_lifecycle.Areas.ProjectManager.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        /// <summary>
        /// GET: /ProjectManager/News?q=technology&sortBy=publishedAt&language=en&pageSize=10
        /// </summary>
            ...
            try
            {
                ...
                if (root.TryGetProperty("articles", out var articles))
                {
                    foreach (var a in articles.EnumerateArray())
                    {
                        ...
                        {
                            Title       = a.TryGetProperty("title",       out var t) ? t.GetString() ?? string.Empty : string.Empty,
                            Description = a.TryGetProperty("description", out var d) ? d.GetString() ?? string.Empty : string.Empty,
                            Url         = a.TryGetProperty("url",         out var u) ? u.GetString() ?? string.Empty : string.Empty,
                            UrlToImage  = a.TryGetProperty("urlToImage",  out var i) ? i.GetString() ?? string.Empty : string.Empty,
                            Source      = a.TryGetProperty("source", out var s) && s.TryGetProperty("name", out var n) ? n.GetString() ?? string.Empty : string.Empty,
